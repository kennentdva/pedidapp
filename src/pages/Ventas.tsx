...
export default function Ventas() {
  ...
  const handleSubmit = async () => {
    ...
    if (itemsFinales.length === 1) {
      ...
      detalleEnviar = { proteina: it.proteina, acompanamientos: it.acompanamientos, sopa: it.sopa, extras: it.extras, nota: it.nota, tipoPlato: it.tipoPlato };
    } else {
      ...
      <div className="flex-1 bg-neutral-900 border border-neutral-800 rounded-3xl p-4 md:p-6 shadow-xl">
        ...
        {showConfig && (
          <div className="mb-8 p-4 bg-black/40 border border-neutral-800 rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6 text-sm">
              <div><h4 className="text-neutral-500 mb-2">Proteínas</h4><div className="flex flex-wrap gap-1 mb-2">{store.menuConfig.proteinas.map(p => (<span key={p} className="bg-neutral-800 px-2 py-1 rounded flex items-center gap-1 text-xs">{p}<button className="text-red-400 ml-1" onClick={() => store.setMenuConfig({ proteinas: store.menuConfig.proteinas.filter(x => x !== p) })}>x</button></span>))}</div><div className="flex gap-1"><input value={newProteina} onChange={e => setNewProteina(e.target.value)} className="w-full bg-neutral-900 border border-neutral-700 rounded p-1 outline-none text-white text-xs" placeholder="Nueva..." /><button onClick={() => { if (newProteina) { store.setMenuConfig({ proteinas: [...store.menuConfig.proteinas, newProteina] }); setNewProteina(''); } }} className="bg-neutral-800 px-2 rounded">+</button></div></div>
        ...
        {activeTab === 'Restaurante' && (
          <div>
            ...
            <div className="mb-8"><h3 className="text-sm font-semibold text-neutral-400 mb-3 uppercase tracking-wider">2. Acompañamientos</h3><div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{store.menuConfig.acompanamientos.map((a: string) => { const c = store.detalle.acompanamientos.includes(a); return (<button key={a} onClick={() => store.toggleAcompanamiento(a)} className={`py-3 px-2 rounded-2xl text-md font-medium transition-all active:scale-95 border-2 ${c ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400' : 'bg-neutral-800 border-transparent text-neutral-400 opacity-60 hover:opacity-100'}`}>{c ? `Con ${a}` : `Sin ${a}`}</button>); })}</div></div>
            <div className="mb-8"><h3 className="text-sm font-semibold text-neutral-400 mb-3 uppercase tracking-wider">3. Sopa</h3><div className="grid grid-cols-2 lg:grid-cols-4 gap-3"><button onClick={() => store.setSopa(null)} className={`py-3 px-2 rounded-2xl text-md font-medium transition-all active:scale-95 border-2 ${!store.detalle.sopa ? 'bg-blue-500 border-blue-500 text-white' : 'bg-neutral-800 border-transparent text-neutral-300'}`}>Sin sopa</button>{store.menuConfig.sopas.map((s: string) => (<button key={s} onClick={() => store.setSopa(s)} className={`py-3 px-2 rounded-2xl text-md font-medium transition-all active:scale-95 border-2 ${store.detalle.sopa === s ? 'bg-blue-500 border-blue-500 text-white' : 'bg-neutral-800 border-transparent text-neutral-300 hover:bg-neutral-700'}`}>{s}</button>))}</div></div>
